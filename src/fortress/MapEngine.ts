...
/**
 * Deterministic Pseudo-Random Number Generator (PRNG)[cite: 6].
 */
...
export class MapEngine {
  public static getGridSize(difficultyLevel: number): number {
    switch (difficultyLevel) {
      case 1: return 12; // 144 Tiles[cite: 6]
      case 2: return 20; // 400 Tiles[cite: 6]
      case 3: return 28; // 784 Tiles[cite: 6]
      case 4: return 34; // 1156 Tiles[cite: 6]
      default: return 20;
    }
  }
  ...
  /**
   * Generates a procedural map with clustered contiguous lakes, mountain ranges, and forests[cite: 6].
   * Supports an optional customGridSize to dynamically scale map dimensions by player count[cite: 6].
   */
  public static generateProceduralMap(
    seed: number, 
    difficultyLevel: number = 1,
    customGridSize?: number
  ): TileState[][] {
    const prng = new SeededPRNG(seed);
    const S = customGridSize || this.getGridSize(difficultyLevel); // Dynamic Map Scale[cite: 6]
    const dMin = Math.floor(S / 5); // Anti-Clustering Spacing Constraint[cite: 6]

    // Target safe structures[cite: 6]
    ...
    // 1. Initialize Baseline Plain Matrix[cite: 6]
    ...
    // Helper: Expand a cluster of terrain from a center point[cite: 6]
      ...
      while (queue.length > 0 && currentSize < targetSize) {
        ...
        if (curr.x >= 0 && curr.x < S && curr.y >= 0 && curr.y < S) {
          if (grid[curr.x][curr.y].terrain === 'PLAINS') {
            ...
            // Push neighboring orthogonal and diagonal tiles to expand blob[cite: 6]
            const neighbors = [
              { x: curr.x + 1, y: curr.y },
              { x: curr.x - 1, y: curr.y },
              { x: curr.x, y: curr.y + 1 },
            ...
            // Shuffle neighbors deterministically using PRNG[cite: 6]
    ...
    // 2. Spawn Contiguous Lakes[cite: 6]
    const minBlob = Math.max(4, Math.floor((S * S) * 0.02)); // ~2% of total map[cite: 6]
    const maxBlob = Math.max(8, Math.floor((S * S) * 0.04)); // ~4% of total map[cite: 6]
    ...
    // 3. Spawn Mountain Ranges[cite: 6]
    ...
    // 4. Spawn Dense Forest Woods[cite: 6]
    ...
    // 5. Spawn Towns & Sanctuaries enforcing Manhattan dMin spacing[cite: 6]
    ...
    // 6. Spawn Citadel surrounded by protective mountain buffer[cite: 6]
    ...
    // 7. Hide 4 Quest Relics on Mountain tiles[cite: 6]
