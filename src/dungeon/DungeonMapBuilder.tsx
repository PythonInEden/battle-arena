...
export function DungeonMapBuilder() {
  ...
  const handleResizeGrid = (newW: number, newH: number) => {
    ...
    setGridHeight(newH);
    setGrid(prev => {
      const next = Array.from({ length: newH }, (_, y) =>
      ...
      if (templateType === 'GREAT_HALL') {
        for (let y = centerY - 1; y <= centerY + 2; y++) {
          for (let x = centerX - 1; x <= centerX + 2; x++) {
            if (next[y]?.[x]) next[y][x] = 'H';
          }
        }
      } else if (templateType === 'ROOM_2X2') {
        for (let y = centerY; y <= centerY + 1; y++) {
          for (let x = centerX; x <= centerX + 1; x++) {
            if (next[y]?.[x]) next[y][x] = selectedBrush === '#' ? '1' : selectedBrush;
          }
        }
        if (next[centerY + 2]?.[centerX]) next[centerY + 2][centerX] = 'd';
      } else if (templateType === 'CHAMBER_3X3') {
        for (let y = centerY - 1; y <= centerY + 1; y++) {
          for (let x = centerX - 1; x <= centerX + 1; x++) {
            if (next[y]?.[x]) next[y][x] = selectedBrush === '#' ? '5' : selectedBrush;
          }
        }
        if (next[centerY + 2]?.[centerX]) next[centerY + 2][centerX] = 'd';
      }
  ...
  // Import Grid from ASCII string
  const handleImport = () => {
    try {
      const cleanedLines = importText
        .split('\n')
        .map(l => l.trim().replace(/^"|",?$|";?$/g, ''))
        .filter(l => l.startsWith('#') || l.startsWith('.') || l.startsWith('1') || l.startsWith('H'));
      ...
      alert('Error parsing map code. Ensure lines contain ASCII characters.');
    }
  };

  // Color lookup per cell
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px', backgroundColor: '#0a1424', padding: '12px 16px', borderRadius: '8px', border: '1px solid #00ffcc' }}>
        <div>
          <h2 style={{ margin: 0, color: '#fff' }}>🛠️ DUNGEON MAP CREATOR</h2>
          <span style={{ fontSize: '12px', color: '#88aaff' }}>Grid: {gridWidth}x{gridHeight} | Drag mouse to draw cells</span>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr', gap: '16px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', backgroundColor: '#070f1e', padding: '12px', borderRadius: '8px', border: '1px solid #112233' }}>
          <div>
            <label style={{ fontSize: '12px', color: '#88aaff', display: 'block', marginBottom: '6px' }}>Grid Size:</label>
            <div style={{ display: 'flex', gap: '6px' }}>
              <button onClick={() => handleResizeGrid(30, 30)} style={{ flex: 1, padding: '4px', backgroundColor: gridWidth === 30 ? '#00ffcc' : '#000', color: gridWidth === 30 ? '#000' : '#00ffcc', border: '1px solid #00ffcc', cursor: 'pointer' }}>30x30</button>
              <button onClick={() => handleResizeGrid(40, 40)} style={{ flex: 1, padding: '4px', backgroundColor: gridWidth === 40 ? '#00ffcc' : '#000', color: gridWidth === 40 ? '#000' : '#00ffcc', border: '1px solid #00ffcc', cursor: 'pointer' }}>40x40</button>
            </div>
          </div>

          <div>
            <label style={{ fontSize: '12px', color: '#88aaff', display: 'block', marginBottom: '8px' }}>Active Brush Tool:</label>
      ...
      {showExportModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ backgroundColor: '#0a1424', border: '2px solid #00ffcc', padding: '24px', borderRadius: '8px', width: '90%', maxWidth: '700px', color: '#fff' }}>
            <h3 style={{ margin: '0 0 10px 0', color: '#00ffcc' }}>📋 Exported Map Blueprint Code</h3>
      ...
      <div style={{ backgroundColor: '#070f1e', border: '1px dashed #00ffcc', padding: '12px', borderRadius: '8px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          <textarea
            ...
            placeholder='Paste ASCII_MAP array lines here (e.g. "#####...", "##11...")'
            rows={2}
