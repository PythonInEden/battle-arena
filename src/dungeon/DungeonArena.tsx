...
export function DungeonArena() {
  ...
  const CLASS_CONFIG: Record<string, { targetGold: number; speed: number; category: string }> = {
    ...
    Fighter: { targetGold: 20000, speed: 5, category: 'Fighter / Combat' },
    Barbarian: { targetGold: 20000, speed: 5, category: 'Fighter / Combat' },
  ...
  return (
    <div style={{ backgroundColor: '#050b14', color: '#00ffcc', fontFamily: 'monospace', minHeight: '100vh', padding: '20px', boxSizing: 'border-box' }}>
      ...
      {/* Header Bar */}
      <header style={{ borderBottom: '2px solid #00ffcc', paddingBottom: '10px', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px', alignItems: 'center' }}>
        <div>
        ...
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          <button
            ...
            style={{ padding: '8px 16px', backgroundColor: viewMode === 'GAME' ? '#00ffcc' : '#000', color: viewMode === 'GAME' ? '#000' : '#00ffcc', border: '1px solid #00ffcc', fontWeight: 'bold', cursor: 'pointer', fontFamily: 'monospace', borderRadius: '4px' }}
          >
            🎮 Play Arena Mode
          </button>
          <button
            onClick={() => setViewMode('BUILDER')}
            style={{ padding: '8px 16px', backgroundColor: viewMode === 'BUILDER' ? '#10b981' : '#000', color: viewMode === 'BUILDER' ? '#fff' : '#10b981', border: '1px solid #10b981', fontWeight: 'bold', cursor: 'pointer', fontFamily: 'monospace', borderRadius: '4px' }}
          >
